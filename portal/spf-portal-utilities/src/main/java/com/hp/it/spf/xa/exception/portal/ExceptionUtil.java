 ...
 * <code>PortalURI</code> object to the Service Portal system and
 * authorization error pages.
 * </p>
 * 
 * @author Sunny Yee, Scott Jorgenson
 * @version TBD
 */
public class ExceptionUtil {
	...
	public static String SESSION_ATTR_AUTH_ERROR_DATA = "SPF_AUTH_ERROR_DATA";
	...
	/**
	 * Create a PortalURI for redirecting to the SPF authorization error secondary
	 * page, and show the given error code in the page (along with the default
	 * title and message for a system error). A default error code is displayed
	 * if null is provided.
	 * 
	 ...
	 * @param errorCode
	 *            An error code of arbitrary format (SPF does not impose a
	 *            format) to display on the authorization error secondary page.
	 * @return PortalURI For redirecting to the authorization error secondary page.
	 */
	static public PortalURI redirectAuthErrorPage(
			PortalContext portalContext, String errorCode) {
		return redirectAuthErrorPage(portalContext, null, errorCode, null);
	}
	 ...
	 *            The portal context.
	 * @param errorTitle
	 *            The title to display on the authorization error secondary page.
	 * @param errorCode
	 *            An error code of arbitrary format (SPF does not impose a
	 *            format) to display on the authorization error secondary page.
	 * @param errorMessage
	 *            The error message to display on the authorization error secondary
	 *            page page.
	 * @return PortalURI For redirecting to the authorization error secondary page.
	 */
	static public PortalURI redirectAuthErrorPage(
		...
		PortalURI authErrorPage = null;
		if (portalContext != null) {
			authErrorPage = portalContext
					.createDisplayURI(Consts.PAGE_FRIENDLY_ID_AUTH_ERROR);
			...
			session.setAttribute(SESSION_ATTR_AUTH_ERROR_DATA, map);
		}
