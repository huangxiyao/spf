 ...
 * <code>&lt;spf-i18n-portal:classicLocaleSelector&gt;</code> tag). The locales
 * expressed are the ones currently available for the portal site, sorted by
 * locale. The locale currently resolved for the user is marked as the
 ...
 * attribute or the <code>labelKey="<i>message-key</i>"</code> attribute. If the
 ...
 * component's resource bundle (if not found, then the <i>message-key</i> itself
 * is used for this purpose). If both <code>label</code> and
 * <code>labelKey</code> are provided, the <code>label</code> attribute takes
 * precedence. It is an error if neither <code>label</code> nor
 * <code>labelKey</code> are provided.
 * </p>
 * </li>
 * <li>
 * <p>
 * The <code>order="<i>spec</i>"</code> attribute lets you optionally specify
 * the sort order used in the list in the selector widget, as well as the
 * display format for each locale name in that list. Use
 ...
 * names as displayed in the list. <b>Note:</b> The <code>order</code> attribute
 * is only relevant for lists that may contain full (ie country-specific
 * locales), since generic (ie language-only) locales have only a language name
 * with which to sort and display.
 * </p>
 * </li>
 * <li>
 * <p>
 ...
 * sorted in ascending order according to their localized locale names (see the
 * <code>order</code> and <code>displayInLocale</code> attributes, above and
 * below); this attribute lets you set the locale for the collating sequence to
 * use in that sort. You pass an <a
 * href="http://www.faqs.org/rfcs/rfc3066.html">RFC 3066</a> language tag for
 * the locale whose collating sequence you want to perform the sort (for
 * example, use <code>sortInLocale="en"</code> to sort according to the English
 * character collating sequence). By default, the sort will be collated
 * according to the user's current locale (the HPWeb standard); or you can
 * explicitly request that by using <code>sortInLocale="current"</code>.
 * </p>
 * </li>
 * <li>
 * <p>
 ...
 * (These localized locale names are also used in sorting the list, where the
 * collating sequence used for the sort is specified by the
 * <code>sortInLocale</code> attribute - see above.) You pass an <a
 * href="http://www.faqs.org/rfcs/rfc3066.html">RFC 3066</a> language tag for
 * the locale (for example, use <code>displayInLocale="en"</code> to display in
 * English). By default, the display will be in the user's current locale (the
 * HPWeb standard); or, instead of passing a language tag, you can explicitly
 * request that by using <code>displayInLocale="current"</code>. Finally, you
 * can use <code>displayInLocale="various"</code> to display the locale name in
 ...
 * alternative ways of specifying a CSS style for the locale selector's label.
 * With the <code>listStyle</code> attribute, you pass an inline CSS style. This
 * should be a string of any CSS properties which are valid properties for
 ...
 * <li>
 * <p>
 * <code>escape="<i>true-or-false</i>"</code> is an optional switch (default:
 * <code>"false"</code>) which if set to <code>"true"</code> will convert any
 * HTML special characters found in the label into their equivalent HTML
 * character entities. <b>Note:</b> Regardless of the value of this switch, the
 * locale options in the list are escaped (though generally they should contain
 * no HTML markup anyway).
 * </p>
 * </li>
 * <li>
 * <p>
 * <code>filterSpan="<i>true-or-false</i>"</code> is another optional switch
 * (default: <code>"false"</code>) which, when set to <code>true</code>, removes
 * any <code>&lt;SPAN&gt;</code> which Vignette may have injected into the
 * label, if any.
 * </p>
 * </li>
 * </ul>
...
public class ClassicLocaleSelectorTag extends LocaleSelectorBaseTag {
    ...
    /**
     * Stores the locale for the <code>displayInLocale</code> attribute from the
     * tag.
     */
    ...
    /**
     * Set the locale for the collating sequence in which to sort, from the
     * <code>sortInLocale</code> attribute in the tag. This converts the tag
     * attribute value (a string) into the desired locale, where the special
    ...
    /**
     * Set the locale to use for locale names, from the
     * <code>displayInLocale</code> attribute in the tag. This display locale is
     * used when localizing the names for sorting purposes, and when
     * subsequently displaying the names in the option list. This converts the
     ...
     * <code>order</code> attribute in the tag. (Note that generic locales - ie
     * language-only locales - are always sorted by language only, and always
    ...
    /**
     * Set the label from the <code>label</code> attribute in the tag. This sets
     * a normalized value (ie trimmed and blank converted to null).
     * 
     * @param label
     *            The label text.
     */
    ...
    /**
     * Set the value from the <code>listStyle</code> attribute. Blank values are
    ...
    /**
     * Set the value from the <code>listClass</code> attribute. Blank values are
     * not normalized to null.
     * 
     * @param value
