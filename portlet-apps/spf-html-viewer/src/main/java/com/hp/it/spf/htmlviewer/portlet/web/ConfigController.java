...
/**
 * <p>
 * The controller class for <code>config</code> mode of the HTML viewer portlet.
 * The HTML viewer is a JSR-286 portlet whose <code>view</code> mode displays
 * the interpolated content of a configured HTML file (called a <i>view
 * file</i>). In <code>config</code> mode, the portlet administrator is able to
 * specify the view file to display, as well as other criteria such as a
...
public class ConfigController extends AbstractController {
     ...
     * <dd>Specifies any include properties to insert into the interpolated view
     * file content during <code>view</code> mode. Its value is set into the
     * model element named
     ...
     * <code>view</code> mode, to launch links in a buttonless child window. Its
     * value is set into the model element named
     ...
     * either <code>true</code> or <code>false</code>. If the option has not yet
     * been set in the preferences, then the default value put into the model is
     * <code>false</code>. The launch-buttonless option is obtained from a
     * render parameter set by the action phase; otherwise it is obtained from
     * the current view data (portlet preferences).</dd>
     * </p>
     * <p>
     ...
     * <dd>If there was any info status to report from the preceding action, the
     * action process will have set the particular status code into the
     ...
     * <code>-1</code>), never cache (value: <code>0</code>), or cache for a
     * positive number of seconds.
     * </p>
     * <p>
     * This method uses WPAP Timber logging to record the outcome. The SPF
     * diagnostic ID, user ID, portlet friendly ID, and overall transaction
     * status are set in all cases. These will be exposed in the WPAP Timber
     * logs as per the logger.xml configuration; the Timber CLIENT_ID is the
     * diagnostic ID, PAGE_ID is the user ID, and PORTLET_NAME is the portlet
     * friendly ID. In the case of an internal error during this controller, the
     * WPAP Timber error logs will include diagnostics about the error.
     * </p>
     * 
     * @param request
     *            The render request.
	...
	if (request.getParameter(ACTION_FLAG) != null) {
		...
		throw new InternalErrorException(request, data.getErrorCode(),
			data.getErrorDiagnostic());
	    }
     ...
     * <code>view</code> or <code>config</code> modes do not have to wait for
     * the cache to naturally expire. The SPF diagnostic ID is included in the
     * WPAP Timber business and error logs.
     * </p>
     * <p>
     * This method uses WPAP Timber logging to record the outcome. The SPF
     * diagnostic ID, user ID, portlet friendly ID, and overall transaction
	...
	try {
	    ...
	    errors.put(Consts.ERROR_CODE_PREFS_UNSAVED, Utils.getDiagnostic(
		    Consts.ERROR_CODE_PREFS_UNSAVED, null));
