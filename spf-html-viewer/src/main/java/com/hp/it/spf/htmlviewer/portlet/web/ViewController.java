 ...
 * The HTML viewer is a JSR-286 portlet. Its <code>view</code> mode displays the
 * proper localized version of a configured HTML file (called a <i>view
 * file</i>), interpolated according to the SPF token language supported by
 * {@link com.hp.it.spf.xa.interpolate.portlet.TokenParser}.
 * </p>
 * <p>
 * In <code>config</code> mode, the portlet administrator is able to specify the
 * HTML file to display, as well as other criteria such as a token-substitutions
 * file for the <code>{INCLUDE:<i>key</i>}</code> token (called an <i>includes
 * file</i>). These are stored as JSR 286 portlet configuration preferences,
 * which this <code>view</code> mode controller accesses in order to identify
...
public class ViewController extends AbstractController {
     ...
     * {@link com.hp.it.spf.htmlviewer.portlet.util.Consts#DEFAULT_INCLUDES_FILENAME}
     * (<code>html_viewer_includes.properties</code>). Interpolation primarily
     * involves applying the SPF token language defined by
     * <code>TokenParser</code>, but also involves rewriting anchor tags
