...
public class PropertyResourceBundleManager {
     ...
     * key is not found in the resource bundle.
     * </p>
     * 
     * @param propertiesName
     *            A properties file name.
     ...
     * properties file cannot be found or has been removed from disk, or if the key is
     * not found in the resource bundle.
     * </p>
     * 
     * @param propertiesName
     *            A properties file name.
    ...
    /**
     * <p>
     * Retrieve a property value (or the given default) from the current resource bundle for the given
     * base properties file, key and locale. This method uses the in-memory
     * "hot" cache (refreshed if file on disk is updated). The properties file
     * is searched for anywhere within the current classpath using the standard
     * system class loader. The search includes looking for the best-fit
     * localized version of the properties file, using the standard search
     * sequence (see ResourceBundle). The method returns the given default if the best-fit
     * properties file cannot be found or has been removed from disk, or if the key is
     * not found in the resource bundle.
     * </p>
     * 
    ...
    private static ResourceBundle setResourceBundleIntoMap(String filename) {
        ...
        try {
            ...
            LOG
                    .error("Cannot load properties file from classpath: "
                            + filename);
            return null;
        }
        if (file != null) {
            try {
                ...
                LOG.error("Problem loading properties file: " + file
                        + "\nException detail: " + e);
                return null;
            }
        } else {
