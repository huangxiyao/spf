...
public class UPSPendingUserRule implements UPSRule {
	
	private static final Logger mLog = Logger.getLogger(UPSPendingUserRule.class);

	public Integer execute(HttpServletRequest request, Map userProfile,
		...
		int status = Constants.STATUS_UPS_PENDING;
		try {
		...
		if (status == Constants.STATUS_UPS_PENDING){
			...
			return new Integer(Constants.ERRORCODE_RULE_UPSPENDING);
		}
		return null;
	}
