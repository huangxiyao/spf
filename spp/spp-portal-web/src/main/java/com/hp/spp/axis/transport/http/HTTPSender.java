 ...
 */
package com.hp.spp.axis.transport.http;
...
public class HTTPSender extends BasicHandler {
   ...
   // protected static Log log = LogFactory.getLog(HTTPSender.class.getName());
    ...
    protected void getSocket(SocketHolder sockHolder,
        ...
        if(log.isDebugEnabled())
        	log.debug("sppmsg:getSocket -> Socket created for Host = "+host +" Port = "+port+" Protocol = "+protocol);
    ...
    private InputStream writeToSocket(SocketHolder sockHolder,
            MessageContext msgContext, URL targetURL,
        ...
        if (useFullURL.value) {
            ...
            header.append((((targetURL.getFile() == null)
                    || targetURL.getFile().equals(""))
                    ? "/"
                    : targetURL.getFile()));
        }
