...
<c:set var="width" value="${160 - (indentLevel * 10)}" />
...
<c:choose>
	<c:when test="${empty title}">
		<tr class="decoration">
			<td align="${left}" width="160" colspan="4"><img src="${spacerUrl}" width="160" height="10" alt=""></td>
		</tr>
		<tr class="decoration">
			<td align="${left}" width="10"><img src="${spacerUrl}" width="10" height="1" alt=""></td>
			<td align="${left}" valign="top" width="150" class="colorCCCCCCbg" colspan="3"><img src="${spacerUrl}" width="140" height="1" alt=""></td>
		</tr>
		<tr class="decoration">
			<td align="${left}" width="160" colspan="4"><img src="${spacerUrl}" width="160" height="10" alt=""></td>
		</tr>	
	...
	<c:otherwise>
		<tr>
			...
			<td align="${left}" width="${width}" colspan="${colspan}">				
				<c:if test="${indentLevel eq 0}"><h2></c:if><c:if test="${!empty url}"><a href="${url}"></c:if>${title}<c:if test="${!empty url}"></a></c:if><c:if test="${indentLevel eq 0}"></h2></c:if>
			</td>
		</tr>
	</c:otherwise>
</c:choose>
